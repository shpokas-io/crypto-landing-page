...
fetch("https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=10&page=1").then((res)=>res.json()).then((data)=>{
    const bitcoinData = data.find((coin)=>coin.id === "bitcoin");
    console.log(bitcoinData);
    if (bitcoinData) {
        const cryptoTableSection = document.querySelector(".hero_crypto-table-grid");
        ...
        coinText.innerHTML = `${bitcoinData.name} <span class="coin-symbol bitcoin-symbol">${bitcoinData.symbol.toUpperCase()}</span>`;
    ...
    } else console.log("Bitcoin data not found.");
});
